...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat is None:
        LOGGER.warning("Received /start update without an effective chat: %s", update)
        return

    await context.bot.send_message(
        chat_id=chat.id,
        text="سلام! متن مورد نظرت رو بفرست تا با Runway برات تصویر بسازم.",
    )
...
async def handle_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message is None or not message.text:
        LOGGER.warning("Received prompt update without a text message: %s", update)
        if update.effective_chat is not None:
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="لطفاً متن دلخواهت رو به صورت پیام متنی ارسال کن.",
            )
        return
    ...
    await message.reply_photo(photo=generation.image_urls[0], caption=caption)
...
async def _run() -> None:
    ...
    try:
        ...
        await updater.wait_until_finished()
